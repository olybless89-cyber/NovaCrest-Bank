...
|---|---|
| Region | Singapore |
| Third-party domain | `app-dptqkburek1u-api-e94GlB86xpEa.gateway.appmedo.com` |
| Method | `POST` |
...
| API Plugin endpoint | `POST https://app-dptqkburek1u-api-e94GlB86xpEa.gateway.appmedo.com/image-to-video/kling-3.0-turbo` |
...
while (true) {
  const queryResponse = await fetch(
    `https://app-dptqkburek1u-api-GaDwzVOqmRNY.gateway.appmedo.com/tasks?task_ids=${encodeURIComponent(taskId)}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
